...
for (const route of ["no/1", "18"]) {
  test(`route ${route}: full content integration preserves introductory prose and verifies original reference titles across toggles`, async () => {
    const source =
...
`);
    const { dom, close } = page(
      ...
      `https://yukicoder.me/problems/${route}`,
    );
    const calls: string[] = [];
