...
[번역·검수 참여 안내](CONTRIBUTING.md)를 참고하세요. 설치 없이 공개 페이지와 GitHub PR로 참여할 수 있으며, 설치 가능한 경우 아래 로컬 검수 도구를 권장합니다. 사람 검수는 문제의 `humanReview`에 검수자 ID 목록으로 기록하고, 최종 PR 병합은 관리자가 담당합니다.
...
문제 MDX의 frontmatter에 `visibility: false`를 지정하면 본문은 유지하되 공개 페이지·문제 목록·확장 기능의 제목 및 본문 번역 대상에서 제외됩니다. 생략하거나 `true`이면 공개 대상입니다. 검수 도구의 **번역 공개** 토글로도 변경할 수 있으며, 비공개 문제는 검수 도구에서 계속 열고 편집할 수 있습니다. 공개 여부 변경은 검수 승인과 별개이며 다음 빌드·배포부터 적용됩니다. Git 저장소의 파일 자체를 비밀로 만드는 설정은 아닙니다.
...
`pnpm package`는 Firefox 0.1.2 업로드용 `dist/archives/extension/yukicoder-ko-firefox-0.1.2.zip`을 만듭니다.
기존 AMO 항목의 새 버전으로 업로드하세요. 확장 ID는 유지하며, AMO 서명·공개 이후 기존 설치에 업데이트가 제공됩니다. ZIP 자체는 서명된 설치 파일이 아닙니다.

Firefox `about:addons`에서 이 확장의 설정을 열면 버전·전체 Git 커밋 SHA·빌드 시각을 확인할 수 있습니다. 툴바 버튼의 번역 켜기/끄기 동작은 유지됩니다.
같은 정보는 패키지 안의 `build-info.json`에도 있으며, ZIP 옆의 `.build.json`에는 해당 ZIP의 SHA-256도 기록됩니다. 패키징할 때 현재 HEAD를 다시 읽지 않고 실제 빌드 시점의 정보를 사용합니다.
미커밋 변경이나 미추적 파일이 있으면 개발 빌드로 표시됩니다. 정식 업로드용 패키지는 변경을 커밋한 깨끗한 작업 트리에서 다시 생성하고, AMO 버전 설명에도 `.build.json`의 `commit`을 적으세요. 문제 번역은 온라인에서 별도로 갱신되므로 설치된 확장 버전의 커밋과 다를 수 있습니다.

0.1.2 변경 사항: 누락된 메인페이지·문제 공개 알림·제출 링크 문구 번역, 비공개 문제의 제목 목록 제외, 설치된 확장의 빌드 기준 표시.
