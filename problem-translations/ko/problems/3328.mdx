...
다음 절차로 만들어지는 무향 그래프를 팔의 개수가 $X$, 팔의 길이가 $(Y_1, Y_2, \cdots, Y_X)$인 **이와이 트리 그래프**라고 정의합니다.
...
이 이와이 트리 그래프의 정점 $i$와 정점 $j$ 사이의 거리를 $f(i,j)$로 정의합니다.
...
팔의 개수가 $4$, 팔의 길이가 $(3,1,4,1)$인 이와이 트리 그래프는 다음과 같습니다.
![](../images/3328/1.png)

이 이와이 트리 그래프에서 $\displaystyle \sum_{i = 0}^{\mathrm{sum}~Y} \sum_{j = i + 1}^{\mathrm{sum}~Y} f(i,j)$의 값은 $134$입니다.
