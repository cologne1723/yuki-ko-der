...
시로군은 어떤 지역으로 여행을 떠나 $K$곳의 목적지를 둘러봅니다. 이 지역은 좌표평면 위에 있으며, $N$개의 역으로 이루어진 철도가 운행되고 목적지가 있습니다. 시로군은 철도와 도보로 목적지와 호텔 사이를 이동합니다.
...
시로군은 소식가이므로 이동 경로에 아직 방문하지 않은 다른 목적지가 있더라도 그곳에서는 식사할 수 없습니다. 따라서 반드시 호텔로 $1$번 돌아온 뒤에 그 목적지를 방문해야 합니다.

시로군의 도보 속도는 $1$이고 철도 이동 속도는 $V=10^{18}$입니다. 거리는 유클리드 거리로 정의하며, 환승 시간은 고려하지 않습니다.
